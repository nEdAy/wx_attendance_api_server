...
@go build -ldflags "-X main.VERSION=1.0.0 -X 'main.BUILD_TIME=%DATE% %TIME%' -X 'main.GO_VERSION=%go_version%' -X main.GIT_HASH=%git_hash%" -o ./build/wx_attendance_api_server.exe ./
exit /B %ERRORLEVEL%
...
set GOARCH=amd64
set GOOS=linux
@go build -ldflags "-X main.VERSION=1.0.0 -X 'main.BUILD_TIME=%DATE% %TIME%' -X 'main.GO_VERSION=%go_version%' -X main.GIT_HASH=%git_hash%" -o ./build/wx_attendance_api_server ./
exit /B %ERRORLEVEL%
...
:clean
@del /S /F /Q "build\wx_attendance_api_server*"
