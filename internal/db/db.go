...
// GetDBMySQL 获取mysql操作对象
func (mysql *DBMySQL) GetDBMySQL() (*gorm.DB, error) {
	if mysql.db == nil {
...
// New 实例化连接MySQL数据库
func New(config *config.MySQLConfig) (*DBMySQL, error) {
	if config == nil {
	...
	mysqlDB := &DBMySQL{db: db, config: config}
